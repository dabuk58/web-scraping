...
def find_cities_by_select(webName):
   readSelect(webName)
   return useModel('fromSelect')

def readSelect(webName):
   js_file = "inputHtmlDiff/readCitiesSelect.js"
   command = ["node", js_file, webName]
   subprocess.run(command, capture_output=True, text=True)

def readInput(webName):
   js_file = "inputHtmlDiff/readCitiesInput.js"
   command = ["node", js_file, webName]
...
def compareAndfindSpecForSuffix(input_file_path, output_file_path, suffix):
   compareSpec(input_file_path, output_file_path+f'.txt')
...
def findSpecInput():
   ...
   input_id = 0
   for i in range(numberOfInputElements):
       count_specializations_result = useModel(f'compared/{i}')
       print(f"\nInputId/countCitieis: {i} / {count_specializations_result}\n")
...
#search selects
count_cities_result = find_cities_by_select(web_name)
if count_cities_result>2:
   print(f"Miasta w select: {count_cities_result}")
